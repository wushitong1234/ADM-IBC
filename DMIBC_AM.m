% DMIBC Alternating Maximization (AM) Algorithm with power constraint
%% output
...
% xn2: (\|x_i\|_{2})^2.   Gam: power constraint.   
% lambda: fixed Lagrange multiplier.
% max_iter: max iteration of AM algorithm

%% main function of DMIBC_AM
function [opt_lm, opt_bot, r_stop, p_out, opttt1, opttt2, zeta, mu, r_phi, r_psi, r_zeta, r_mu, t] = DMIBC_AM(d, s, xn2, Gam, lambda, max_iter)
    % known variables
    ...
    % unknowm variables
    p = zeros(m,1);     % marginal distribution of X   m*1
    q = zeros(k,1);     % marginal distribution of Y   k*1  
    ...
    while (iter < max_iter)  && (r_stop > 1e-8)
        iter = iter + 1
        ...
        % Step 1: update P_X  (p, \mu)
        % Solve F(\mu) = 0 with Newton's method
        temp2 = Ffun(0,tp,xn2,Gam);
        if temp2 <= 0 || m <= 4 || Gam == 100    % without power constraint
            x2 = 0;
            r_mu(iter) = 2e-18;
        else
            r_mu(iter) = abs(Ffun(mu,tp,xn2,Gam));
            x1 = mu;
            x2 = x1-Ffun(x1,tp,xn2,Gam)/Ffun_d(x1,tp,xn2);
            t2 = 0; % iter of Newton's method 2
            while (abs(x2-x1) > 1e-10) && (t2 < nt_iter2)
                t2 = t2+1;
                x1 = x2;
                x2 = x1-Ffun(x1,tp,xn2,Gam)/Ffun_d(x1,tp,xn2);
            end
        ...
        mu_old = mu;    mu = x2;    
        mu(mu<0) = 0;   mu(find(isnan(mu))) = 1;
        K_mu = exp(-mu*xn2);      K_mu(K_mu<1e-100) = 1e-100;

        % update P_X -p
        tpp = tp .* K_mu;
        p = tpp / sum(tpp);
        % compute P_Y -q
        q = s' * p;     q(q<1e-100) = 1e-100;

        % Step 2: update P_{Z|Y}, P_Z (t, r)
        tt1 = K_ze' * phi .* psi;
        tt2 = ((p .* s)' * d )./ q;     
        ...
        % Step 3:  compute the LM rate (phi psi zeta)
...
% F(mu)
function fw1 = Ffun(x,tp,xn2,Gam)
    Ku = exp(-x*xn2);
    f0 = tp' * Ku;
    f1 = tp' * (xn2 .* Ku);
    fw1 = -Gam + f1/f0;
